...
A partial derivative tells you the rate of change of a function of several variables in one of the positively oriented cardinal directions. There a lot like the derivatives that you learn about for functions of one variable in Calc I.

### Plain old ordinary derivatives

A derivative, as you probably know, measures the instantaneous rate of change of a function value with respect to its input variable. It's defined by the difference quotient:
$$
...
Now, imagine that you're sitting on a hillside whose elevation is given by the graph of a function $z=f(x,y)$. There's not just one oriented direction anymore; rather, there's infinitely many. Later, you'll talk about *directional* derivatives which really do measure the rate of change in *any* direction. The first step, though, is to consider what happens if you march in the one of the two positively oriented cardinal directions - i.e. due North or due East.
...
Partial derivatives are defined using multidimensional difference quotients:
...
Computing partial derivatives is fairly easy; I bet you've done it before! In a Calc I class, you might very well look at a function like
$$
...
OK, the function is really really $f(x,y)=x^2\,\sin(x+y^2)$ but that's not the point.
</details>
...
We can push the idea of a partial derivative of a bivariate function further in a couple ways.

### Functions of more variables

There's no reason we can't talk about functions of three or more variables. Maybe,
$$
...
An important application on Calc I is the computation of a tangent line. In that context, we use the point slope formula 
$$
y = y_0 + m(x-x_0),
$$
...
Optimization is a fundamental application of calculus that is of tremendous importance. It's possible that we're getting slightly ahead of the game here but if you see this more than once that's fine. We're only just starting the topic, too; there's lots more to do, including classification and constrained optimization.

### Univariate optimization 

In Calc I, you certainly learn how to find maxima and minima of functions of a single variable. If you've got a differentiable function $f$ defined on $\mathbb R$, the strategy is to solve the equation $f'(x)=0$; these solutions are called *critical points* and they are candidates for where extremes might occur. 

Once you've got your critical points, it can be a bit tricky to determine which are maxima, which are minima, and which are neither. There's a second derivative test that can help with this classification. The easiest thing to do, though, is to examine a graph and that's what we'll do here.

### Bivariate optimization

When studying the optimization of functions of two variables, *both* derivatives should be zero to obtain a critical point. That yields a system of equations to solve for the critical points. There's again a second derivative test to help us classify the critical points, though we won't get into that now. It's generally pretty easy to tell what's going on from a contour diagram
...
Judging from @fig-contour_for_optimization1, it sure looks like there's a minimum value with small, positive coordinates.
...
It certainly looks like there's a saddle point at the origin and a maximum in the second quadrant that's harder to pinpoint exactly.
