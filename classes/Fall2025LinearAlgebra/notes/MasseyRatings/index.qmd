...
$$M\vec{r} = \vec{p}$$
can itself be written down and interpreted without reference to $X$ or $y$. As we've already noted, $M_{ii}$ denotes the number of games played by team $i$ and $M_{ij}$ is the opposite of the number of games played by teams $i$ and $j$. Similarly, the entries of $\vec{p}$ are the set of score differences for each team.

For example, if we expand the first term in the product $M\vec{r}$ and set that equal to the first entry of $\vec{p}$, we get
$$4r_0 - 2r_1 - 2r_2 = 7.$$
...
//| echo: false
viewof name = {
  const names = ["---", "Audrey", "Amina","Bryan P","Bryan M","Carlos","Collin","Estelle","Fin","Gabriel","Isabel","Jacob C","Jacob S","James","Jordan","Lauren","Leo","Lexi","Lilia","Miranda","Sal","William","Zachary"];
  year_conf_pairs.slice(0,names.length).forEach((o,i) => o['name'] = names[i]);
  return Inputs.select(year_conf_pairs.slice(0,names.length), {format: o => o.name, label: "Name:"})
}
```
