...
The serious, mathematical study of probability theory has its origins in the $17^{\text{th}}$ century in the writings of [Blaise Pascal](https://en.wikipedia.org/wiki/Blaise_Pascal) who studied - gambling. Many elmentary examples in probability theory are still commonly phrased in terms of gambling. As a result, it's long standing tradition to use coin flips, dice rolls and playing cards to illustrate the basic principles of probability. Thus, it does help to have a basic familiarity with these things.
...
- An event is some subset of the the sample space.
...
Suppose I flip a coin that comes up heads 70% of the time and I also roll a four sided die with the with the probabilities 
...
$$
The possible outcomes are $X$ is $1$, $2$, or $3$. The random variable can be defined by simply listing its values 
...
Note the all the probabilities should be non-negative and they should sum to one.
...
The variance of our weighted die roll in example above is
...
We'd like to understand the way that binomial random variables are distributed - that is, the binomial distribution. To do so, we'll first derive the distribution of a single binomial trial; we can then derive the binomial distribution using rules of combination for random variables.
...
Here's what that fit looks like, when we model a binomial distribution with the correpsonding normal:
