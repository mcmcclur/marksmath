...
We've got a feed forward neural network with $1$ input layer, $N$ hidden layers, and $1$ output layer. We suppose that layer $i$ has $n_i+1$ nodes numbered $0,1,\ldots,n$ and we want to describe how the values propagate from layer $i-1$ to layer $i$.
...
$$
This produces a real number. The sigmoid maps that to the open interval $(0,1)$, with zero mapping to the midpoint $1/2$, negative numbers mapping to the bottom half, and positive numbers to the top. Finally, rounding yields a $0$ or $1$ result as desired.

Perhaps a simpler way to describe the result is that the model produces

\begin{aligned}
0 & \quad \text{ when } y = w_1 \, x_1 + w_2 \, x_2 + w_0 < 0 \text{ and } \\ 
1 & \quad \text{ when } y = w_1 \, x_1 + w_2 \, x_2 + w_0 > 0.
\end{aligned}
...
y = 2 \, x_1 + 2 \, x_2 - 3
$$
and classify according to whether $y<0$ or $y>0$. To see this in action is a simple matter of plugging in:
$$
\begin{aligned}
